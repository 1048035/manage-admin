<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <!-- <el-form-item label="消耗积分类型类型" prop="scoreType">
        <el-select v-model="queryParams.scoreType" placeholder="请选择消耗积分类型类型" clearable>
          <el-option
    ...
    <!-- <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['fhorder:orderScore:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['fhorder:orderScore:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['fhorder:orderScore:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['fhorder:orderScore:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row> -->

    <el-table v-loading="loading" :data="orderScoreList" @selection-change="handleSelectionChange">
        <el-table-column type="expand" >
            <template slot-scope="props">
                <el-form label-position="left" class="demo-table-expand">
                            ...
                            <el-form-item label="消耗积分类型类型" style="margin-bottom: 0px">
                                <span>
      ...
      <!-- <el-table-column label="消耗积分类型类型" align="center" prop="scoreType">
        <template slot-scope="scope">
...
<script>
import { listOrderScore, getOrderScore, delOrderScore, addOrderScore, updateOrderScore } from "@/api/fhorder/orderScore";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listOrderScore(this.queryParams).then(response => {
        this.orderScoreList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getOrderScore(id).then(response => {
        this.loading = false;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateOrderScore(this.form).then(response => {
            ...
            addOrderScore(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除积分订单编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delOrderScore(ids);
      }).then(() => {
        this.loading = false;
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('fhorder/orderScore/export', {
        ...this.queryParams
      }, `orderScore_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
